...
export default class Patientpost extends React.Component {
  constructor(props) {
     super(props);
     this.state=({radio1:true, radio2:false, first_check:false});
   }
  componentWillMount(){
    ...
    if (this.props.filtercall.filter1 && this.props.filtercall.filter2 && this.props.filtercall.filter3) {
      ...
      return (
        <div className="">
          ...
          {
            [1,2,3,4].map((item0, i)=>{
              return (<div className="unit_div row" key={item0}>
                {
                  array_universal.slice((item0-1)*4,((item0)*4)).map((item1, i)=>{  //16-4-8,8-12,12-16
                    return (
                      <div className="unit_div col-sm-3" key={item1.newid}>
                        <Postheaderfirst user={item1.username} date={item1.date} url={item1.url}></Postheaderfirst>
