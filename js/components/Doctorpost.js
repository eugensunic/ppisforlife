...
export default class Doctorpost extends React.Component {
  ...
  aboutWritting(){
    ...
    <div className="headerbar" style={{marginBottom:6}}>
      <p>This page shows comment posts where users talk about their doctor’s opinion on the drug, whether the drug is good or not good if taken short-term/long-term. Users express what their doctor told them for their therapy. The user associated doctor tag refers to this.</p>
      <p>The explicit doctor tag refers to doctors alone, by themselves expressing their opinion on the drug such as concerns about their long-term usage, dosage etc.</p>
  ...
  return(
    <div className="">
      ...
      {this.state.first_check?this.aboutWritting():(<Headerbardoctor></Headerbardoctor>)}
      {
        [1,2,3,4].map((item0, i)=>{
              ...
              array_universal.slice((item0-1)*4,((item0)*4)).map((item1, i)=>{  //16-4-8,8-12,12-16
                return (
                  <div className="unit_div col-sm-3" key={item1.id}>
                    <Postheaderfirstdoctor user={item1.assoc_username} url={item1.url} opinion={item1.flag_doc} doc_alone={item1.docalone}></Postheaderfirstdoctor>
                    <br/>
                    <Postmain bg_color={this.props.bg_color_post} content_cite={item1.doctors_tought} number={item1.id}></Postmain>
