...
export default class Postfooter extends React.Component {
  ...
  arrayToTag(arr){
    ...
    arr.map((item0, i)=>{
      return(
        <p className="post_tagvalue_rest" key={i}>{item0}</p>);
    }));
  }
render(){
  return(
    <div className="post_footer_div">
      <div className="post_tagtoggle" onClick={()=>{this.setState({open_tag:!this.state.open_tag})}}>{this.state.open_tag?"close":"open"}</div>
      {this.state.open_tag?<div>
