...
$stmt = $conn->prepare("SELECT post.id, sides.name AS side,  `condition`.name AS  `condition` , conditionppi.name AS ppi_condition, nutrient.name AS nutrient, drug_brand.name
FROM post,  `condition` , post_condition, post_sides, sides, conditionppi, post_conditionppi, nutrient, post_nutrient,data, drug_brand
...
AND data.drug_brand_FK=drug_brand.id
AND data.dosage >= ? AND data.dosage <= ?
