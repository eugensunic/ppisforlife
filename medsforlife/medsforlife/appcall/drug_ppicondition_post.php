...
$stmt = $conn->prepare("SELECT DISTINCT post.id as id0, content_cite, url, date, username, conditionppi.name
FROM post, conditionppi, post_conditionppi
WHERE `conditionppi`.id = post_conditionppi.`conditionppi` 
AND post_conditionppi.post = post.id
AND `conditionppi`.name LIKE ?
